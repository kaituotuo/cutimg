(function (root, factory) {
  ...
  const messages = {
    zh: {
      ...
      productName: "长图分割",
      privacyTitle: "图片内容仅在当前浏览器处理，不上传到 Cutimg 图片处理服务",
      localProcessing: "本地处理",
      policyLink: "隐私与许可",
      policyDialogTitle: "隐私、地域访问与许可",
      policyLocalHeading: "图片处理",
      policyLocalBody: "你选择、拖放或粘贴的图片只在当前设备的浏览器内读取、切割并生成下载文件。图片内容不会上传到 Cutimg 的图片处理服务，也不会被 Cutimg 保存。",
      policyRegionHeading: "地域访问",
      policyRegionBody: "Cutimg 暂不向中国大陆地区提供服务。Cloudflare 会依据边缘网络识别的国家代码拦截 CN 请求；VPN、代理或错误的地理识别可能导致实际结果不同。",
      policyHostingHeading: "托管与访问数据",
      policyHostingBody: "为选择默认语言，页面可能请求同源接口获取两位国家/地区代码。该接口不接收图片内容，函数本身不读取或记录 IP；Cloudflare 仍可能依据其隐私政策处理访问日志、IP 和安全元数据。",
      policyUsageHeading: "使用提示",
      policyUsageBody: "请确认你对处理的图片拥有合法使用权。超大图片会受到浏览器内存和 Canvas 限制，服务按现状提供，不保证所有设备都能成功导出。",
      policyLicenseHeading: "开源许可",
      policyLicenseBody: "项目源代码采用 MIT License；界面图标使用 Lucide，适用 ISC 及其中注明的 Feather MIT 许可。",
      privacyFileLink: "隐私说明",
      noticeFileLink: "许可与素材",
      closePolicy: "关闭",
      imageActions: "图片操作",
      reuploadImage: "重新上传",
      removeImage: "删除当前图片",
      ...
      guidePosition: "分割线位置",
      deleteGuide: "删除所选分割线",
      deleteGuideAt: "删除第 {index} 条分割线（{position} px）",
      slicesLabel: "切片数量",
    ...
    en: {
      ...
      privacyTitle: "Image content is processed only in this browser and is not uploaded to Cutimg's image-processing service",
      localProcessing: "Local processing",
      policyLink: "Privacy & licenses",
      policyDialogTitle: "Privacy, regional access & licenses",
      policyLocalHeading: "Image processing",
      policyLocalBody: "Images you choose, drop, or paste are read, split, and turned into downloads only in your current browser. Image content is not uploaded to Cutimg's image-processing service and is not stored by Cutimg.",
      policyRegionHeading: "Regional access",
      policyRegionBody: "Cutimg is not available in mainland China. Cloudflare blocks requests identified as CN by its edge country code; VPNs, proxies, or inaccurate geolocation may produce different results.",
      policyHostingHeading: "Hosting and access data",
      policyHostingBody: "The page may request a same-origin endpoint for a two-letter country code to choose the default language. The endpoint does not receive image content, and the function itself does not read or record IP addresses. Cloudflare may still process access logs, IP addresses, and network metadata under its privacy policy.",
      policyUsageHeading: "Use note",
      policyUsageBody: "Make sure you have the right to process the images you use. Very large images are subject to browser memory and Canvas limits; the service is provided as-is and export is not guaranteed on every device.",
      policyLicenseHeading: "Open-source licenses",
      policyLicenseBody: "The project source code is under the MIT License. Interface icons use Lucide under the ISC License, including the noted MIT license for Feather-derived icons.",
      privacyFileLink: "Privacy notice",
      noticeFileLink: "Notices & assets",
      closePolicy: "Close",
      ...
      newSplit: "New split",
      uploadToBegin: "Open a long image to begin",
      ...
      zoomOut: "Zoom out",
      zoomIn: "Zoom in",
      uploadHeading: "Open a long image to start splitting",
